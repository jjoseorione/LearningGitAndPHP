...
<html>
  ...
  <body>
    ...
    <form action="./deleteExe2.php" method="post">
      <table class="tabla1">
        <tr>
          <th>🔘</th>
          <th>ID</th>
        ...
        <?php
          $conexion = mysqli_connect("localhost", "usuario_curso_php", "cursophp", "curso_php")
            or die("Error en la conexión a la base de datos");
          mysqli_set_charset($conexion, "utf8");
          $consulta = mysqli_query($conexion, "SELECT * FROM alumnos") or
            die("Error en la consulta. Consulta vacía");
          while($fila = mysqli_fetch_array($consulta))
          {
            echo "<tr>
                  <td><input type=\"radio\" name=\"borrarReg\" value=$fila[id]></td>
                  <td>$fila[id]</td>
                  <td>$fila[nombre]</td>
                  <td>$fila[mail]</td>
      ...
      <input type="submit" value="Eliminar" class="abotonB">
    </form>
  </body>
</html>